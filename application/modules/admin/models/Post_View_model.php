...
class Post_View_model extends CI_Model{
	...
	var $column_order = array('vw.viewId','t1.title','t2.userName','t2.email','t1.description'); //set column field database for datatable orderable
	var $column_search = array('t1.title','t2.userName','t2.email'); //set column field database for datatable searchable
	var $order = array('vw.viewId'=>'DESC'); // default order
	var $where = array();
	...
	function prepare_query(){
		...
		$this->db->join(USER_POST.' as t1','t1.postId = vw.postId');
		$this->db->join(USER.' as t2','t2.userId = vw.userId');
